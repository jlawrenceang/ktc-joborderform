...
### 2026-06-09
- **Admin approval review:** the `/admin/approvals` broker cards now show a pre-approval review — valid-ID-on-file status plus Agreement version and Terms / Data-Privacy consent badges with dates (green ✓ / amber ⚠). Queries the `brokers` consent columns; `AdminRow` gained an `extra` slot.
