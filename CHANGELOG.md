...
- **Phase 1 remediation — 7 go-live blockers (T1) fixed** (2026-06-28), from the 2026-06-28 process/coherence audit:
  - **T1-01** "Continue with Google" no longer silently no-ops — the button is gated behind a runtime flag (`VITE_GOOGLE_OAUTH_ENABLED`, hidden until the provider is configured) and any OAuth error is now surfaced (`AuthRail.tsx`).
  - **T1-02** a name change now clears `valid_id_path`, so `/verify-id` shows the upload prompt again (was a dead-end where the admin re-reviewed the OLD ID against the new name; also re-pings the approvals desk) (`Account.tsx`).
  - **T1-03** all "you can file Job Orders while you wait / held / Draft / up to 10" copy rewritten to the verify-only truth (pending accounts **cannot** file; the held model was retired by `0163`) across the manual, Lara chat nodes, Home, MyJobOrders, the onboarding tour, banners, VerifyId, and their Tagalog mirrors; dead held UI made inert.
  - **T1-04** rejected consignee requests are recoverable — migration `0185` lets a requester **see** their own rejected rows (RLS) and **resubmit** them to pending (`resubmit_consignee` now accepts `rejected`), with a Resubmit affordance in My Requests (was bricked by the global unique-name index).
  - **T1-05** a dedicated **"Collect RPS at the window"** cashier bucket, independent of base-payment state — a walk-in assessed RPS after the X-ray payment was confirmed can now be settled (`CashierStation.tsx`).
  - **T1-06** pending (verify-only) customers now have an in-app support channel — `/support` is whitelisted for pending and the verify-only panel shows a contact card + Get-help link (`open_ticket` already allows pending) (`Shell.tsx`).
  - **T1-07** an `AGREEMENT_VERSION` bump now forces re-consent — a new version-aware gate in `ProtectedRoute` blocks customers on a stale version and re-stamps via `record_agreement_consent` (new `ReConsent` screen; staff exempt). `0162`'s `has_recorded_consent()` only checked non-null, so a bump never re-gated.
- **Coherence doc-sync — 31 mismatches (C-01..C-31)** (2026-06-28): runtime-wins fixes for doc/code/spec/memory drift, chiefly the **ADR-0035 ops-overhaul** lag — Administration/Cashier-Station/Job-Order-Lifecycle cores, ADR-0016/0018/0035 dated addenda, the six-role matrix, version/migration snapshots (now `0185` / `v1.6.75`), `types.ts` role comment, and the `tooling-inventory` script/Management-API entries. Also fixed a dangling `docs/adr/README.md` reference to the removed local `/adr`.
- **Owner failsafe hardened: email-keyed backstop** (2026-06-28): migration `0184` — `is_owner()` and `is_admin()` now also return true when the JWT email (`auth.email()`) is the owner's, so a **missing or flag-wrong `customers` row** (auth-user delete-cascade, or a bad migration/manual edit) can no longer lock the owner out of the back office. This closes a real gap in the "owner cannot be locked out" non-negotiable: the gates previously decided access purely from the `is_owner`/`is_admin` column. MFA is still enforced (the backstop defeats data corruption, not the owner's own 2FA), and the email is a GoTrue-set JWT fact, not spoofable to a confirmed address. Triggered by the 2026-06-28 roast (owner magic-link hit a pending gate) — but the read-only prod dig found the **owner account is healthy**; the roast had minted a link for the old, now-**rejected** `jla.ktcport@gmail.com`. Also: corrected the stale "`jla.ktcport` = admin fallback" claim (it's now a rejected customer; there is currently **one** privileged account) across `System Scale`, ADR-0004 (dated amendment), and smoke-tests 02/04/05/06; documented break-glass recovery (`seed-owner.sql`) in the **Owner Failsafe** concept. A secret-question backdoor was considered and **rejected** (a second master key bypassing 2FA); a second owner for redundancy was offered and deferred. Backend migration + docs; no frontend change.
- **Repo framework mirror synced to JLA framework v1.1** (2026-06-28): removed the stale repo-local `/adr` command (`.claude/commands/adr.md`) — it was the verbose pre-v1.0 version and was shadowing (reverting) the improved lean global `/adr`; the repo now inherits the global command. Updated `docs/agent/tooling-inventory.md`: the Commands section now points at the global framework roster (owned by the global `CLAUDE.md` Index, not restated here) instead of listing a local copy, and the Skills section records **Jarvis** (v1.1's experimental author-independent verify/advise sentinel) with this repo flagged as a designated dogfooding target. Single-owner honored — no global roster duplicated into the repo. Docs/harness-only; no runtime or DB change.
