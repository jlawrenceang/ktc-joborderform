...
const SERVICES: { key: string; title: string; body: string }[] = [
  {
    key: 'jo',
    title: 'Job Orders',
    body: 'Request special and value-added services — X-Ray inspection, DEA exam, OOG stripping — and track each order through to completion.',
  },
  {
    key: 'release',
    title: 'Container Release & Pull-out',
    body: 'File the delivery documents, settle the assessed charges online, and claim your Official Receipt at the KTC office for pull-out.',
  },
  {
    key: 'pay',
    title: 'Online Payments',
    body: "View the computed charges and KTC's bank and QRPH details, then upload your payment for the cashier to confirm.",
  },
  {
    key: 'vessel',
    title: 'Vessel Schedule & Rates',
    body: 'Check the current vessel calls and last free days, and estimate charges with the rate calculator.',
  },
]
    ...
    <div style={{ display: 'grid', placeItems: 'center', minHeight: '100%', padding: 24 }}>
      <div className="ktc-glass ktc-rise" style={{ width: '100%', maxWidth: 620, padding: '32px 34px 28px' }}>
        ...
        <p style={{ margin: '8px 0 0', fontSize: 15, lineHeight: 1.55, color: 'hsl(var(--ink-2))' }}>
          {t('The online service desk of KTC Container Terminal Corp. — for accredited customs brokers and KTC staff to file and track terminal and port-services work.')}
        </p>
        ...
        <div style={{ marginTop: 22, display: 'grid', gap: 12 }}>
          ...
          <p className="ktc-label" style={{ margin: 0, fontSize: 12.5, lineHeight: 1.55 }}>
            {t('Customs brokers: create an account to begin accreditation. KTC staff accounts are invite-only. For assistance with access, please contact KTC customer service or visit the KTC office.')}
          </p>
        </div>

        {/* Footer */}
        <div style={{ marginTop: 22, paddingTop: 14, borderTop: '1px solid var(--glass-brd)', display: 'flex', flexWrap: 'wrap', gap: '4px 16px', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ display: 'inline-flex', gap: 16, flexWrap: 'wrap' }}>
            <a href="/customer-info-sheet.html" target="_blank" rel="noopener" className="ktc-link" style={{ fontSize: 12 }}>
              {t('Customer Information Sheet (print / download)')}
            </a>
            <Link to="/agreement" className="ktc-link" style={{ fontSize: 12 }}>{t('Customer Agreement')}</Link>
          </span>
