...
- [ ] **Google Sheets link (view + entry)** — keep Supabase as source of truth (a Sheet bypasses RLS/caps/guards → no live two-way sync). Build a one-way **app→Sheet mirror** (read-only) for checking + add missing operational fields to the in-app JO form and/or a **bounded admin import** for data entry. Decide which fields, who may import, mirror cadence.
