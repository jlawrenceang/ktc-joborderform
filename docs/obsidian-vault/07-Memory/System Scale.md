...
| Metric | Count |
|--------|-------|
| App version | `src/version.ts` = **`v1.6.73`** (live on `portal.ktcterminal.com`). Footers show version + git commit + build date |
| Migrations | **174 files**, `0001_init` … **`0183_audit_closure_notify_rexray_consignee_vessel`** — all applied + tracked in `public._migrations`. Numbering is split across a **portal lane** and a **fuel lane** (fuel = `0135`/`0140`/`0150`), so there are gaps; the portal lane now runs contiguously through `0183`. Recent big runs: consignee approval gate + CIS (`0165`–`0169`), the **ADR-0035 ops overhaul** (`0170`–`0177`: SoD · auto-complete · queue lifecycle · priority + re-X-ray lanes · ops→cashier billing · invoice-gated payment), and the **whole-app audit closure** (`0178`–`0183`). Earlier (`0160`–`0164`): support_contact SELECT, Google-OAuth finish-registration, consent enforcement, pending verify-only lockdown, disposable-email block |
