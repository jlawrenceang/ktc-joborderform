...
## 2026-06-30 — ADR-0037 charges cutover SHIPPED + go-live hardening DEPLOYED (v2.0.8, migrations 0228+0229)

**The cutover the 06-29 entry below deferred is now LIVE, and two codex-review hardening passes are deployed on top. `APP_VERSION` = `v2.0.8`; migrations through `0229` applied to prod (`0228`+`0229` applied 2026-06-30 via `_apply_one.mjs`, verified live).** The `charges`/`payment_orders` spine is the live operational money path — the old base/RPS/supplement flow is retired. Fully break-tested (gates / RLS / RBAC / writes hold). See memory `cutover-shipped-go-live-status` + [[target-architecture-jo-payment-invoice]].
