...
- **Roles & security:** cashier/checker roles + role-permission matrix, TOTP 2FA for admin/owner (server-enforced aal2, `0049`), single session per account — last-login-wins + dead-session RLS cut-off (`0054`/`0055`), idle timeouts everywhere (customer 30 min / staff 60 min) with "still there?" prompt, auto-suspend + owner alerts on escalation attempts (`0046`–`0048`), security headers/CSP in `vercel.json`, Logs tab + 15-min ops watchdog + System health panel (`0045`).
