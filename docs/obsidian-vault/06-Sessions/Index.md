...
- [[2026-06-21 Release Module + ERP Link + ktcportal Rename]] — customer-filed **release / pull-out** module (migrations **0124–0126**, [[ADR-0024]]): `release_orders` (consignee + BL + DO/BL → CSR doc verify → charges → QRPH pay → cashier confirm → OR/released), DO verification now live; **additional charges** (set-once base + `release_supplements`, OR blocked until all confirmed); **ERP link** = combined Record-OR captures the OR no. + ERP service-invoice control no. (`service_invoice_no`); **cancel** (customer+staff, pre-payment) + upfront approval gate; X-ray queue → ops view (`view_xray_queue`, 0123). Plus de-Jotform + GitHub repo rename to **ktcportal**.
