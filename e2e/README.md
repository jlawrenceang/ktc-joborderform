...
- `smoke.spec.ts` — **Phase 1, active (10 tests).** Unauthenticated smoke: routing, login render, protected-route redirects, SPA rewrite, public legal pages (IRR/Terms/Privacy), registration consent gate, Turnstile mounts. Runs without logging in, so the server-side CAPTCHA doesn't block it.
